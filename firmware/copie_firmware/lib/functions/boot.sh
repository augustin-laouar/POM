	...
	root=$1
	{
		if grep -q overlay /proc/filesystems; then
			kernel_major_version=$(uname -r|awk -F. '{print $1}')
				...
				mount | grep -q "/sp_rom" && {
					mount -t overlay -o noatime,lowerdir=/sp_rom:/,upperdir=$1/upper,workdir=$1/work "overlayfs:$1/upper" /mnt && root=/mnt
				} || { 
					mount -t overlay -o noatime,lowerdir=/,upperdir=$1/upper,workdir=$1/work "overlayfs:$1/upper" /mnt && root=/mnt
				}
			else
				mount | grep -q "/sp_rom" && {
					mount -o noatime,lowerdir=/,upperdir=/sp_rom -t overlayfs "overlayfs:$1" /mnt
					mount -o noatime,lowerdir=/mnt,upperdir=$1 -t overlayfs "overlayfs:$1" /mnt && root=/mnt
				} || { 
